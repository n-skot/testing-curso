...
test('test obj', () => {
  const data = { name: 'Ricardo' }; // Para testear objetos se usa .toEqual.
  data.lastname = 'Molina'; // Manipulación del objeto.
  expect(data).toEqual({ name: 'Ricardo', lastname: 'Molina' }); // Espero que el objeto de arriba sea igual a este.
});
...
test('string', () => {
  ...
  expect('Christoph').toMatch(/stop/); //<-- De esta forma usamos expresiones regulares
});
